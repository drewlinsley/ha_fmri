function roi_tuning = characterize_tuning_cukor(data,masks,model,clusters,labels,ims)
%Load images
...
data = mean(cat(4,data{:}),4);

[~,hmodel] = max(model,[],2);
roi_tuning = cell(numel(masks),1);
for idx = 1:numel(masks), %iterate through masks,
    data_mat = zeros(size(data,3),numel(clusters{idx}));
    for il = 1:size(data,3),
        it_d = data(:,:,il);
        data_mat(il,:) = it_d(masks{idx}==1);
    end
    tc = clusters{idx};
    uni_clusts = unique(tc);
    num_clusts = numel(uni_clusts);
    mask_tuning = zeros(size(labels,1),num_clusts);
    for il = 1:num_clusts,
        figure,
        it_data = data_mat(:,tc == uni_clusts(il));
        it_vals = zeros(numel(labels),size(it_data,2)); %average resp X voxels
        for iil = 1:numel(labels),
            mod_inds = hmodel == iil;
            it_vals(iil,:) = mean(it_data(mod_inds,:));
        end
        it_vals = max(it_vals,[],2); %Max across cluster
        it_vals(isnan(it_vals)) = 0; %set NaNs to 0
