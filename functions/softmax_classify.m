function true_stats = softmax_classify(training_data,testing_data,training_labels,testing_labels,null,num_iter)

%Initialize seed
%rng(1,'twister');
if ~exist('null','var'),null = false; end
if ~exist('num_iter','var'),num_iter = 1e4; end
%Standardize data
tr_mu = mean(training_data);
tr_sd = sqrt(var(training_data));
training_data = bsxfun(@rdivide,bsxfun(@minus,training_data,tr_mu),tr_sd);
testing_data = bsxfun(@rdivide,bsxfun(@minus,testing_data,tr_mu),tr_sd);
num_classes = numel(unique(training_labels));
input_size = size(training_data,2);
lambdas = [.01,.1,1,10];
num_l = numel(lambdas);
options.MaxIter = 500;
kfold = 5;
lambda_holder = zeros(kfold,1);

c = cvpartition(training_labels,'Kfold',kfold);
for idx = 1:kfold,
    it_train_data = training_data(c.training(1),:)';
    it_test_data = training_data(c.test(1),:)';
    it_train_lab = training_labels(c.training(1))';
    it_test_lab = training_labels(c.test(1))';
    temp_num_classes = numel(unique(it_train_lab));
    perfs = zeros(num_l,1);
    for il = 1:num_l
        softmaxModel = softmaxTrain(input_size, temp_num_classes, lambdas(il), ...
                                    it_train_data, it_train_lab, options);
        pred_labels = softmaxPredict(softmaxModel, it_test_data);
        perfs(il) = mean(it_test_lab == pred_labels);
    end
    [~,lambda_holder(idx)] = max(perfs);
end

lambda = lambdas(mode(lambda_holder));
softmaxModel = softmaxTrain(input_size, num_classes, lambda, ...
                            training_data', double(training_labels)', options);
[pred_labels,true_stats.scores] = softmaxPredict(softmaxModel, testing_data');
true_stats.accuracy = mean(testing_labels==pred_labels'); %classification accuracy
[true_stats.cm,true_stats.order] = confusionmat(double(testing_labels),pred_labels'); %confusion matrix
true_stats.pred_labels = pred_labels;
if null == true,
    perm_acc = zeros(num_iter,1);
    for pi = 1:num_iter,
        null_labels = testing_labels(randperm(numel(testing_labels)));
        perm_acc(pi) = mean(null_labels==pred_labels'); %classification accuracy
    end
    true_stats.perm_CI = [prctile(perm_acc,97.5),prctile(perm_acc,2.5)];
    true_stats.acc_pval = ((sum(true_stats.accuracy < perm_acc) + 1) ./ (num_iter + 1));
end
...
function [labels,mscores,scores] = softmaxPredict(softmaxModel, data)
