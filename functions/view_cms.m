function reshaped_cms = view_cms(input,cv_inds,labels)

reshaped_cms = cell(max(cv_inds),1);
for idx = cv_inds,
    ti = input{idx};
    mask_cms = cell(numel(ti),1);
    for il = 1:numel(ti), %iterate through masks
        tim = ti{il};
        full_cm = zeros(numel(labels),numel(labels),numel(tim));
        for iil = 1:numel(tim), %iterate through clusters
            tim_cc = tim{iil};
            tim_cm = tim_cc.cm;
            tim_order = tim_cc.order;
            for x = 1:numel(tim_order),
                for y = 1:numel(tim_order),
                    full_cm(tim_order(x),tim_order(y),iil) = tim_cm(x,y);
                end
            end
            figure,
            imagesc(full_cm(:,:,iil));
            axis square
            if isfield(tim_cc,'acc_pval'),
                title(sprintf('Cluster %i; Accuracy = %.2f%%, p = %.2f',iil,trace(full_cm(:,:,iil)) ./ sum(sum(full_cm(:,:,iil))),tim_cc.acc_pval))
            else
                title(sprintf('Cluster %i; Accuracy = %.2f',iil,trace(full_cm(:,:,iil)) ./ sum(sum(full_cm(:,:,iil)))))
            end
            set(gca,'XTick',1:numel(labels),'XTickLabels',labels)
            set(gca,'YTick',1:numel(labels),'YTickLabels',labels)
            xticklabel_rotate([],45,[],'Fontsize',8);
        end
        mask_cms{il} = full_cm;
    end
    reshaped_cms{idx} = mask_cms;
end
