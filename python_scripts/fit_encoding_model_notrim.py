...
def extract_1d_voxels (voxel_volume):
    ...
    num_vox = voxel_volume.shape
    num_vox = num_vox[0] * num_vox[1]
    vox_it = voxel_volume.shape[2]-1
    voxel_array = np.zeros((r_size[2],num_vox))
    for idx in range(0,vox_it):
        curr_vox = np.isnan(voxel_volume[:,:,idx])
        this_data = voxel_volume[:,:,idx]
        voxel_array[idx,:] = np.reshape(this_data, (1,-1))
...
def shift_voxels (voxel_timecourse, run_ids, hdr_est, model, trim_amount):
    ...
    for idx in range(0,num_events-1):
        ...
        mean_pad = np.ones((hdr_est,num_vox)) * np.mean(old_vec,0) #create mean pad
        print mean_pad.shape
...
def filter_data (X,TR):
    ...
    for idx in range(0,num_vox):
        if ~np.isnan(np.sum(X[:,idx])):
            T = TimeSeries(X[:,idx], sampling_interval=TR)
...
matplotlib.pyplot.imshow(rh_1_array)
matplotlib.pyplot.show()
matplotlib.pyplot.imshow(rh_2_array)
matplotlib.pyplot.show()
...
matplotlib.pyplot.plot(np.mean(rh_vox_r[:,0:3],1))
matplotlib.pyplot.show()

#1. Find nonnan voxel ids relative to full map
#2. Identify which
empty_array = np.zeros((rh_1.shape[0],rh_1.shape[1]))
for idx in range(0, suprathresh.shape[0]):
    (x,y) = ind2sub(empty_array.shape,suprathresh[idx])
    empty_array[x,y] = np.mean(rh_vox_r[idx,0:3])
